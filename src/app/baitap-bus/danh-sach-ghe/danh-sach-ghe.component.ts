...
export class DanhSachGheComponent implements OnInit {
  ...
  choose(ghe) {
    if (ghe.trangThaiChon) {
      this.dsGheDangChon.push(ghe.ghe);
    }
    else {
      this.dsGheDangChon.map((item, index) => {
        if (ghe.ghe.SoGhe === item.SoGhe)
          this.dsGheDangChon.splice(index, 1);
      })
    }
    this.mangGheDangChon = this.dsGheDangChon;
  }
  huyGhe(SoGhe) {
    this.mangGheDangChon.map((item, index) => {
      if (item.SoGhe === SoGhe) {
